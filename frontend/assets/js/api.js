...
const DEFAULT_TILE_CONFIG={
  url:'https://{s}.api.tomtom.com/maps/orbis/map-display/tile/{z}/{x}/{y}.png?apiVersion=1&style=street-light&key={key}',
...
export async function getMapTileConfig(){
  if(!mapTileConfigPromise){
    mapTileConfigPromise=api('/routes/map-tile-config').catch(err=>{
      console.warn('Failed to load map tile config, using defaults:',err);
      return DEFAULT_TILE_CONFIG;
    });
  }
  return mapTileConfigPromise;
}
export function createTileLayer(overrides={}){
  const layer=L.tileLayer(DEFAULT_TILE_CONFIG.url,{
    attribution:DEFAULT_TILE_CONFIG.attribution,
    subdomains:DEFAULT_TILE_CONFIG.subdomains,
    maxZoom:DEFAULT_TILE_CONFIG.maxZoom,
    key:'',
    ...overrides
  });
  getMapTileConfig().then(cfg=>{
    if(cfg&&cfg.url&&cfg.url!==layer._url){
      layer.setUrl(cfg.url);
    }
  }).catch(()=>{});
  return layer;
}
