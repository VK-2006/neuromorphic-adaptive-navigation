(function(){
  ...
  function init(){
    ...
    addEventListener('resize',resize,{passive:true});host.addEventListener('pointermove',move,{passive:true});const onVisibility=()=>{if(!document.hidden)start()};addEventListener('visibilitychange',onVisibility);const io='IntersectionObserver'in window?new IntersectionObserver(e=>{visible=e[0]?.isIntersecting??true;if(visible)start()},{threshold:.03}):null;io?.observe(host);start();
    state={dispose(){running=false;cancelAnimationFrame(raf);raf=0;io?.disconnect();removeEventListener('resize',resize);removeEventListener('visibilitychange',onVisibility);removeEventListener('navora:theme',syncTheme);host.removeEventListener('pointermove',move);scene.traverse(o=>{o.geometry?.dispose?.();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>{Object.values(m).forEach(v=>v?.isTexture&&v.dispose());m.dispose?.()})});renderer.dispose();renderer.forceContextLoss?.();renderer.domElement.remove();state=null}}
  }
