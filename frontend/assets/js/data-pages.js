...
async function history(){
  ...
  try{
    ...
    h.innerHTML=rows.length?rows.map(j=>`<tr><td><a href="journey-replay.html" data-replay="${esc(j?._id)}">${when(j?.createdAt)}</a></td><td>${esc(j?.mode||'—')}</td><td>${j?.averageRisk!=null?Math.round(100*(1-Number(j.averageRisk)))+'%':'—'}</td><td>${Number(j?.reroutes)||0}</td><td>${esc(j?.status||'—')}</td></tr>`).join(''):'<tr><td colspan="5">No journeys yet.</td></tr>';
...
async function notifications(){
  ...
  try{
    ...
    h.querySelectorAll('[data-read]').forEach(b=>b.addEventListener('click',async()=>{
      try{await api(`/notifications/${encodeURIComponent(b.dataset.read)}/read`,{method:'PATCH'});b.style.opacity='.65'}catch(e){toast(e.message,'error')}
    }));
