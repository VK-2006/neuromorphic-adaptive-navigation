...
async function load(){
  try{
    ...
    if(window.Chart&&canvas&&trend.length){
      try{
        new window.Chart(canvas,{type:'line',data:{labels:trend.map(x=>x?.label||''),datasets:[{label:'Historical safety',data:trend.map(x=>num(x?.safety)),tension:.35,fill:false}]},options:{responsive:true,plugins:{legend:{display:true}},scales:{y:{min:0,max:100}}}});
        empty?.classList.add('hidden');
      }catch(e){empty?.classList.remove('hidden');toast(`Chart unavailable: ${e.message}`,'warning')}
    }else empty?.classList.remove('hidden');
  }catch(e){
    ['metric-safety','metric-memory','metric-success','metric-avoided'].forEach(id=>set(id,'—'));
