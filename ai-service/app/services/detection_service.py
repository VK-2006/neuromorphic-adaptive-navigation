...
class Detector:
    def __init__(self):
        self.model=None;self.mode='development/heuristic-fallback';self.version='detector-dev-1';self.validated=False;self.runtime_ready=False;self.targets=list(DEFAULT_TARGETS);self.load_error=None;self.validation_issues=[];self.unready_weights_present=False
        ...
        readiness=model_validation_status('detector',settings.detector_weights,settings.metadata_path)
        self.validation_issues=list(readiness.get('reasons') or [])
        if torch is not None and settings.detector_weights.exists():
            try:
                ...
                self.runtime_ready=bool(readiness.get('passed'))
                if self.runtime_ready:
                    self.model=candidate;self.mode='torchscript-trained-weights-functional'
                else:
                    self.model=None;self.unready_weights_present=True;self.mode='development/heuristic-fallback-unready-weights'
            except Exception as e:
                self.model=None;self.runtime_ready=False;self.load_error=f'{type(e).__name__}: {e}';self.mode='development/heuristic-fallback-load-error'
        self.validated=False
        self.hog=cv2.HOGDescriptor();self.hog.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())
    def detect(self,image):
        if self.model is not None and self.runtime_ready:
            try:return self._torchscript(image)
            except Exception as e:
                self.load_error=f'runtime {type(e).__name__}: {e}';self.mode='development/heuristic-fallback-runtime';self.model=None;self.runtime_ready=False
    ...
    def _fallback_detect(self,image):
        ...
        for (x,y,bw,bh),conf in zip(boxes,weights):out.append({'objectClass':'person','confidence':float(min(1,conf)),'boundingBox':[x/w,y/h,bw/w,bh/h],'approximateDistance':float(max(1,45*(1-bh/h))),'metadata':{'detector':'opencv-hog','validated':False,'runtimeReady':True}})
        ...
        if candidates:
            _,x,y,bw,bh=max(candidates);out.append({'objectClass':'road damage','confidence':.42,'boundingBox':[x/w,y/h,bw/w,bh/h],'approximateDistance':8.0,'metadata':{'detector':'opencv-development-heuristic','validated':False,'runtimeReady':True}})
        return out[:12]
    def _torchscript(self,image):
        ...
        if isinstance(raw,(list,tuple)) and raw and isinstance(raw[0],dict):
            ...
            for (x1,y1,x2,y2),score,label in zip(boxes,scores,labels):
                ...
                idx=int(label)-1;cls=self.targets[idx] if 0<=idx<len(self.targets) else 'unknown';out.append({'objectClass':cls,'confidence':float(score),'boundingBox':[float(x1/640),float(y1/384),float((x2-x1)/640),float((y2-y1)/384)],'approximateDistance':None,'metadata':{'detector':'torchvision-scripted','validated':False,'runtimeReady':True}})
            return out[:50]
        ...
        for x1,y1,x2,y2,score,idx in rows:
            if float(score)<.3:continue
            i=int(idx);cls=self.targets[i] if 0<=i<len(self.targets) else 'unknown';out.append({'objectClass':cls,'confidence':float(score),'boundingBox':[float(x1/640),float(y1/384),float((x2-x1)/640),float((y2-y1)/384)],'approximateDistance':None,'metadata':{'detector':'torchscript-row-contract','validated':False,'runtimeReady':True}})
        return out[:50]
detector=Detector()
