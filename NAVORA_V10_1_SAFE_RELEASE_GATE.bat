...
chcp 65001 >nul
title NAVORA V10.1 - SAFE RELEASE GATE
...
start "NAVORA V10.1" cmd.exe /d /k ""%~f0" __NAVORA_INNER__"
exit /b 0
...
set "RELEASE_BAT=NAVORA_V10_1_SAFE_RELEASE_GATE.bat"
...
echo.
echo ================================================================================
echo   NAVORA V10.1 - SAFE RELEASE / EXTERNAL GATE VERIFIER
echo ================================================================================
echo Repo: "%REPO%"
echo.
...
set "RUN_LOG=%REPO%\logs\V10_1_LAST_RUN.txt"
> "%RUN_LOG%" echo NAVORA V10.1 RUN
...
if errorlevel 1 (
  ...
  git commit -m "chore: harden Navora V10 Windows release gate"
  if errorlevel 1 (
    set "LAST_RC=!errorlevel!"
    goto :fatal
  )
...
choice /C YN /N /M "Did a REAL OTP email arrive and did resend/verify/reset work? [Y/N] "
if errorlevel 2 (
  set "OTP_GATE=PENDING"
  set /a PENDING+=1
) else (
...
set "REPORT=%REPO%\logs\V10_1_EXTERNAL_GATE_RESULT.txt"
> "%REPORT%" echo NAVORA V10.1 EXTERNAL GATE RESULT
>>"%REPORT%" echo ============================================================
...
echo.
echo ================================================================================
echo   NAVORA V10.1 RESULT
echo ================================================================================
echo Commit: !COMMIT!
echo Google real login:            !GOOGLE_GATE!
echo Real OTP mailbox:             !OTP_GATE!
echo Physical GPS/camera/WebRTC: !HARDWARE_GATE!
...
if "!PENDING!"=="0" (
  echo [PASS] V10.1 AUTOMATED + ALL REAL EXTERNAL GATES PASSED.
  set "FINAL_RC=0"
) else (
  echo [PASS] V10.1 SOFTWARE / GITHUB / EXACT RENDER DEPLOYMENT PASSED.
  echo [PENDING] !PENDING! real external gate(s) still require evidence.
  set "FINAL_RC=2"
)
