...
if(pages.length!==25)throw new Error(`V8 page contract expected 25 pages, found ${pages.length}`);
...
function mock(path,method,page){
  ...
  if(['/api/v1/admin/users','/api/v1/admin/hazards','/api/v1/admin/chat/reports','/api/v1/admin/audit'].includes(path))return{status:200,data:[]};
