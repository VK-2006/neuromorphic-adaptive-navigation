...
def main():
    ...
    args=ap.parse_args()
    backend=base(args.backend); ai=base(args.ai); host=urllib.parse.urlparse(backend).hostname
    errors=[]; warnings=[]
    ...
    for path,name,needle in [
        ...
        ("/register.html","Register page","Navora"),("/journey.html","Journey page","Navora"),
        ("/manifest.json","PWA manifest",None),("/service-worker.js","Service worker",None)
    ]:
        try:
    ...
    try:
        ...
        d=unwrap(obj); google=d.get("google",{}) if isinstance(d,dict) else {}; passkeys=d.get("passkeys",{}) if isinstance(d,dict) else {}; email=d.get("email",{}) if isinstance(d,dict) else {}
        ok("Google client configuration") if google.get("enabled") is True and google.get("clientId") else fail("Google client configuration","GOOGLE_CLIENT_ID missing")
        ok("WebAuthn production configuration",f"rpId={host}") if passkeys.get("enabled") is True and passkeys.get("rpId")==host else fail("WebAuthn production configuration",repr(passkeys))
        ok("Brevo credential configuration") if email.get("brevoConfigured") is True else fail("Brevo credential configuration","BREVO_API_KEY and BREVO_SENDER_EMAIL required")
    except Exception as exc: fail("Auth configuration",str(exc))
    ...
    try:
        ...
        ok("Traffic provider selection","tomtom") if isinstance(d,dict) and d.get("activeTraffic")=="tomtom" else fail("Traffic provider selection",repr(d))
    ...
    try:
        ...
        else: fail("Geocoding capability status",repr(d))
    except Exception as exc: fail("Geocoding capability status",str(exc))

    try:
        _,_,obj=call("GET",backend+"/api/v1/traffic/status",timeout=45)
        d=unwrap(obj)
        ok("TomTom credential readiness") if isinstance(d,dict) and d.get("provider")=="tomtom" and d.get("live") is True else fail("TomTom credential readiness",repr(d))
    except Exception as exc: fail("TomTom credential readiness",str(exc))
    ...
    except Exception as exc: fail("Live geocoding",str(exc))

    payload={"source":{"lat":17.3850,"lng":78.4867},"destination":{"lat":17.4375,"lng":78.4483},"simulation":False}
    try:
        _,_,obj=call("POST",backend+"/api/v1/routes/compare",payload,timeout=120,retries=3)
        d=obj.get("data") if isinstance(obj,dict) else None; routes=d.get("routes") if isinstance(d,dict) else None
        if isinstance(routes,list) and routes and d.get("recommendedRouteId"): ok("Live route comparison",f"{len(routes)} route(s)")
        else: fail("Live route comparison",repr(obj)); routes=[]
        live_tt=[r for r in routes if r.get("trafficProvider")=="tomtom" and r.get("trafficMode")=="live"]
        ok("Live TomTom annotation",f"{len(live_tt)} route(s)") if live_tt else fail("Live TomTom annotation",repr([{"id":r.get("id"),"mode":r.get("trafficMode"),"error":r.get("trafficError")} for r in routes]))
    except Exception as exc: fail("Live route comparison",str(exc))

    try:
        stamp=int(time.time()*1000); _,raw,_=call("GET",f"{backend}/socket.io/?EIO=4&transport=polling&t={stamp}",timeout=45)
