...
const server=http.createServer((req,res)=>{
  ...
  if(u.pathname.startsWith('/api/')){
    ...
    res.end(JSON.stringify({success:false,message:'Local V9 harness API fallback'}));
    return;
  }
  let file=null;
...
server.listen(0,'127.0.0.1',()=>{
  ...
  console.log(`LOCAL V9 FUNCTIONAL HARNESS: ${base}`);
  const child=spawn(process.execPath,[path.join(__dirname,'browser_v9_functional_e2e.js'),base],{
    cwd:ROOT,
    stdio:'inherit',
    env:{...process.env,NAVORA_V9_LOCAL_HARNESS:'1'}
  });
