...
Navora's policy floors. `--mark-validation` additionally requires a passing data gate whose
held-out SNN CSV SHA-256 matches the exact file evaluated here.
"""
...
def main():
    ...
    ap.add_argument('--min-samples',type=int,default=SNN_EVAL_MINIMUMS['minSamples']);ap.add_argument('--min-accuracy',type=float,default=SNN_EVAL_MINIMUMS['minAccuracy']);ap.add_argument('--min-macro-f1',type=float,default=SNN_EVAL_MINIMUMS['minMacroF1'])
    ap.add_argument('--seed',type=int,default=2026);ap.add_argument('--mark-validation',action='store_true',help='Write riskValidated only when metric + data-gate policy is satisfied')
    a=ap.parse_args();configured={'minSamples':a.min_samples,'minAccuracy':a.min_accuracy,'minMacroF1':a.min_macro_f1}
