...
assert not (ROOT / 'frontend/assets/js/devices.js').exists(), 'Device controller script must be removed from the runtime'
assert 'navigator.bluetooth' not in read('frontend/assets/js/journey.js'), 'journey runtime should not depend on device-controller APIs'
...
need('frontend/service-worker.js', '/journey.html', '/shared-journey.html')
assert '/devices.html' not in read('frontend/service-worker.js')
print('V9_CONTRACTS PASS: active NAVORA journey, share flow, and PWA shell match the current camera-free runtime.')
