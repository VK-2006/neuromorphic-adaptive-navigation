...
exports.readiness=async(req,res)=>{
  ...
  const detectorRuntimeReady=info?.detector?.runtimeReady===true || info?.detector?.mode==='browser-local-coco-ssd';
  const riskValidated=info?.riskModel?.validated===true;
  const aiSafetyEligible=aiReachable&&detectorRuntimeReady&&riskValidated;
  const warnings=[];
  ...
  else if(!detectorRuntimeReady)warnings.push('Detector runtime artifact/readiness is unavailable; trained detector perception cannot be used.');
  else if(!riskValidated)warnings.push('SNN risk model is not scientifically validated; automatic live safety decisions remain gated.');
  ...
  ok(res,{fieldMode:true,routing:{provider:env.routingProvider,live:routingLive},traffic:{provider:env.trafficProvider||null,live:trafficLive},ai:{reachable:aiReachable,detectorRuntimeReady,detectorValidated:false,riskValidated,safetyEligible:aiSafetyEligible,mode:info?.detector?.mode||info?.riskModel?.mode||'unavailable',detectorScientificValidationRequired:false},simulationCapability:env.simulationMode,requireValidatedAiForLive:env.liveRequireValidatedAi,warnings});
};
