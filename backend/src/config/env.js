const path=require('path');const crypto=require('crypto');require('dotenv').config({path:process.env.ENV_FILE||path.resolve(process.cwd(),'.env')});
const num=(v,d)=>Number.isFinite(Number(v))?Number(v):d;const bool=(v,d=false)=>v==null?d:String(v).toLowerCase()==='true';
const env={
  nodeEnv:process.env.NODE_ENV||'development',port:num(process.env.PORT,5000),
  mongoUri:process.env.MONGODB_URI||'mongodb://127.0.0.1:27017/navora',
  jwtAccessSecret:process.env.JWT_ACCESS_SECRET||crypto.randomBytes(48).toString('base64url'),
  jwtRefreshSecret:process.env.JWT_REFRESH_SECRET||crypto.randomBytes(48).toString('base64url'),
  ...
  rerouteCooldownSeconds:num(process.env.REROUTE_COOLDOWN_SECONDS,45),simulationMode:bool(process.env.SIMULATION_MODE,false),liveRequireValidatedAi:bool(process.env.LIVE_REQUIRE_VALIDATED_AI,true),
...
if(env.nodeEnv==='production'&&(!process.env.JWT_ACCESS_SECRET||!process.env.JWT_REFRESH_SECRET))throw new Error('Production JWT secrets must be configured');
module.exports=env;
